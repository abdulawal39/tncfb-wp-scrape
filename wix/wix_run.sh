#!/usr/bin/env bash
# Pull Wix sites from HTTP Archive: query -> BigQuery table -> GCS CSV
...
OUT="${OUT:-wix_sites.csv}"
LOCATION="${LOCATION:-US}"
DATASET="${DATASET:-tncfb_wix}"
TABLE="${TABLE:-wix_${DATE//-/_}}"
BUCKET="${BUCKET:-${PROJECT}-tncfb-wix}"

SQL="$(sed "s/@DATE/${DATE}/g" wix.sql)"
