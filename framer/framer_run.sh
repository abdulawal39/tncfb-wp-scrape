#!/usr/bin/env bash
# Pull Framer sites from HTTP Archive: query -> BigQuery table -> GCS CSV
...
OUT="${OUT:-framer_sites.csv}"
LOCATION="${LOCATION:-US}"
DATASET="${DATASET:-tncfb_framer}"
TABLE="${TABLE:-framer_${DATE//-/_}}"
BUCKET="${BUCKET:-${PROJECT}-tncfb-framer}"

SQL="$(sed "s/@DATE/${DATE}/g" framer.sql)"
