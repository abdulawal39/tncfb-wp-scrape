#!/usr/bin/env bash
# Pull Shopify sites from HTTP Archive: query -> BigQuery table -> GCS CSV
...
OUT="${OUT:-shopify_sites.csv}"
LOCATION="${LOCATION:-US}"
DATASET="${DATASET:-tncfb_shopify}"
TABLE="${TABLE:-shopify_${DATE//-/_}}"
BUCKET="${BUCKET:-${PROJECT}-tncfb-shopify}"

SQL="$(sed "s/@DATE/${DATE}/g" shopify.sql)"
