#!/usr/bin/env bash
# Pull WordPress sites from HTTP Archive: query -> BigQuery table -> GCS CSV
# shards -> download + merge locally.
#
# Usage:
...
OUT="${OUT:-wordpress_sites.csv}"
LOCATION="${LOCATION:-US}"
DATASET="${DATASET:-tncfb_wordpress}"
TABLE="${TABLE:-wordpress_${DATE//-/_}}"
BUCKET="${BUCKET:-${PROJECT}-tncfb-wordpress}"

SQL="$(sed "s/@DATE/${DATE}/g" wordpress.sql)"
