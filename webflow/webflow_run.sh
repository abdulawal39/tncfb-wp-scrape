#!/usr/bin/env bash
# Pull Webflow sites from HTTP Archive: query -> BigQuery table -> GCS CSV
...
OUT="${OUT:-webflow_sites.csv}"
LOCATION="${LOCATION:-US}"
DATASET="${DATASET:-tncfb_webflow}"
TABLE="${TABLE:-webflow_${DATE//-/_}}"
BUCKET="${BUCKET:-${PROJECT}-tncfb-webflow}"

SQL="$(sed "s/@DATE/${DATE}/g" webflow.sql)"
